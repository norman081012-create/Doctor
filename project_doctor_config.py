...
* [Phase 2: 廣泛排除期 (DDx Rule-Out)]：OPQRST 達標，展開鑑別診斷，對可能但非首要懷疑的疾病進行防禦性排除。
  ...
  (1) [高敏感度要求 SnNout]：所依據的陰性所見，必須來自該疾病的【高敏感度指標】。低敏感度指標的陰性【不具排除力】（例：「無手抖」對甲狀腺機能亢進為低敏感度陰性，不得用以排除）。
  (2) [非典型亞型檢查 — 強制自問]：在降權前，【必須】明確列出該疾病的非典型／亞型表現，並逐一確認目前的陰性所見是否也能否證這些亞型。若任一亞型仍能解釋病人現況，【禁止降權】。
      範例：體重上升、無手抖，僅否證了典型交感亢進型甲亢，【無法】否證淡漠型甲亢 (apathetic hyperthyroidism)，故不得排除。
...
* Phase 2：啟動次要 DDx 的 Rule-Out。未通過 Rule-Out 三條件者，列入升階佇列。
