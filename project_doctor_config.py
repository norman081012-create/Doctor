...
<soap_s>
極度簡短！必須掃描並統整「所有」歷史對話脈絡 (Chat History)，絕對不可遺漏先前的資訊。直接將全局累積的主訴與問診資訊進行英文醫學術語翻譯與條列化，不要添加多餘的連綴詞。必須加入並嚴格依照 OPQRST (Onset, Provocation/Palliation, Quality, Region/Radiation, Severity, Time) 框架結構化統整病患主訴。
</soap_s>
<soap_a>
採用「症狀群：臆斷與鑑別診斷」的映射格式撰寫，精簡條列。
【強制分類與結案格式】：請嚴格根據「口語問診狀態」給予標籤：
1. 尚未問診：標記為 `DDX [診斷]`
2. 已問診且症狀不吻合 (低度懷疑)：標記為 `r/o [診斷] need further [客觀處置/檢查]`
3. 已問診且症狀吻合 (高度懷疑)：標記為 `suspected [診斷] need further [客觀處置/檢查]`

【強制排序規則】：
- 第一順位 (問診狀態)：suspected > r/o > DDX
- 第二順位 (嚴重度)：高致命性/高嚴重度疾病 > 低嚴重度疾病
</soap_a>
<soap_p>
記錄臨床處置與下一步計畫，必須嚴格紀錄以下項目並極簡條列化：
