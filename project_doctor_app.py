...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.caption(f"💡 低於 {bd_lower_limit} 分時，防禦機制將強行突破專業面具進行極端邏輯毒打。")
        
        mf_upper_limit = st.slider("MF 顧問面具疲勞上限", min_value=0, max_value=100, value=85, step=5)
        st.caption(f"💡 高於 {mf_upper_limit} 分時，解構溫和引導模式，自動啟動冷酷阻斷。")
...
def main():
    ...
    with col_chat:
        ...
        if user_input := st.chat_input("請輸入病患主訴或回應..."):
